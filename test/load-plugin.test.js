...
test('successfully load a plugin with sync function', (t) => {
  ...
  const plugin = new Plugin(fastq(app, app.loadPluginNextTick, 1), function (instance, opts, done) {
    done()
  }, false, 0)

  app.loadPlugin(plugin, function (err) {
...
test('successfully load a plugin with async function', (t) => {
  ...
  const plugin = new Plugin(fastq(app, app.loadPluginNextTick, 1), async function (instance, opts) { }, false, 0)
...
test('catch an error when loading a plugin with async function', (t) => {
  ...
  const plugin = new Plugin(fastq(app, app.loadPluginNextTick, 1), async function (instance, opts) {
...
test('successfully load a plugin when function is a Promise, which resolves to a function', (t) => {
  ...
  const plugin = new Plugin(fastq(app, app.loadPluginNextTick, 1), new Promise(resolve => resolve(function (instance, opts, done) {
    done()
  })), false, 0)
...
test('successfully load a plugin when function is a Promise, which resolves to a function, which is wrapped in default', (t) => {
  ...
  const plugin = new Plugin(fastq(app, app.loadPluginNextTick, 1), new Promise(resolve => resolve({
